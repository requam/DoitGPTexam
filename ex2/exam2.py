...
def main():
    def extract_csv(response):
        if ';' in response:
            print("Received CSV format data")
            res_line = response.split("\n")
            csv_data = []
    ...
    def save_csv(dataframe):
        file_path = filedialog.asksaveasfilename(defaulextension=".csv", filetypes=[("CSV file", "*.csv")])
        if file_path:
            dataframe.to_csv(file_path, index=False)
            print(f"Save CSV file to {file_path}")
        else:
            print("Do not save CSV file")
